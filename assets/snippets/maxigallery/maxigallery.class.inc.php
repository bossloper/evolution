...
class maxiGallery {
	...
	function createthumb($filename,$filetype,$path_to_gal,$prefix="",$resize=true) {
		...
		if($resize){
			$sizes = explode('x', $max_thumb_size); //uxello fix for php 5.3
			if (count($sizes) > 1) {
	...
	// uxello
	function updateEditedOn($docid) {
		global $modx;
		// uxello mod to update page update time
		if ($docid) {
			$sqlupdateeditedon = "UPDATE ".$modx->getFullTableName("site_content")." SET editedon=" . time() . " WHERE id=$docid";
			$rs=$modx->db->query($sqlupdateeditedon);
			
			if ($_SESSION['mgrInternalKey']) {
				$sqlupdateeditedon = "UPDATE ".$modx->getFullTableName("site_content")." SET editedby=".$_SESSION['mgrInternalKey']." WHERE id=$docid";
				$rs=$modx->db->query($sqlupdateeditedon);
			}
		}
	}
