		...
		if($_REQUEST['action']=='upload_pics') { //if "upload pictures" has been used
			...
			if(!file_exists($mg->path_to_gal)) {
				if(!$mg->mgconfig['use_ftp_commands']) {    
					$old_umask = umask(0);
					//uxello change default file permissions
					if(!mkdir($mg->path_to_gal, 0755)) {
					...
					$old_umask = umask(0);
					//uxello change default file permissions
					$setPerm = ftp_site($connect, 'CHMOD 0755 /'.$path_to_galleries.$mg->pageinfo['id'].'/');
					if (!$setPerm) {
						$output = 'Could not set permissions: '.'CHMOD 0755 /'.$path_to_galleries.$mg->pageinfo['id'].'/';
					}
					umask($old_umask);
					ftp_close($connect);
				} 
			...
			for($i=0;$i<$mg->mgconfig['upload_field_count'];$i++) { //for each of the upload fields:
				...
				if($name!="") {
					...
					if(move_uploaded_file( $_FILES['file'.$i]['tmp_name'] , $mg->path_to_gal.$name )) {
						chmod($modx->config['base_path'].$mg->path_to_gal.$name,0644);
					...
					/* uxello fix for No such file or directory error
			...
			//uxello delete images if not in database and after any resyn (probably failed upload due to very large image crashing php)
			if(file_exists($mg->path_to_gal)) {
				$rsx=$modx->db->query("SELECT * FROM " . $mg->pics_tbl . " WHERE gal_id='" . $mg->pageinfo['id'] . "'");
				...
				while ($name = readdir($dh)) { //for each file in the gallery directory if not in database then delete
					if ($name != '..' && $name != '.' && strpos($name, 'tn_') !== 0 && !isset($filesExist[$name])) {
						if(file_exists($mg->path_to_gal.$name)) unlink($mg->path_to_gal.$name);	
					}
				}
				closedir($dh);
			}
